...
export const MOVIE_RECOMMANDATIONS_URL = async (id) => {
  const response = await axios.get(`${REQUEST_URL}/movie/${id}/recommendations?api_key=${API_KEY}&language=en-US&page=1`);
  return response;
};
...
export const PERSON_DETAILS_URL = async (id) => {
  const response = await axios.get(`${REQUEST_URL}/person/${id}?api_key=${API_KEY}&language=en-US`);
  return response;
};
...
export const PERSON_MOVIE_CREDITS_URL = async (id) => {
  const response = await axios.get(`${REQUEST_URL}/person/${id}/movie_credits?api_key=${API_KEY}&language=en-US`);
  return response;
};
